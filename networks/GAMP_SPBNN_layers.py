...
class PrunedGAMPModule(nn.Module):
    ...
    def update_importance(self):
        """Update importance scores based on weight magnitude"""
        with torch.no_grad():
            self.theta_importance += torch.abs(self.theta)
            self.phi_importance += torch.abs(self.phi)

    def apply_pruning_mask(self, current_epoch):
        """Apply pruning mask"""
        if not self.apply_pruning or current_epoch < self.pruning_warmup:
            return

        # Progressive pruning rate
        progress = min(1.0, (current_epoch - self.pruning_warmup) / 100)
        current_pruning_rate = self.pruning_rate * progress

        with torch.no_grad():
            # Prune theta - ensure at least one dimension is retained
            if current_pruning_rate > 0:
                theta_flat = self.theta_importance.view(-1)
                k = max(1, int(current_pruning_rate * theta_flat.numel()))
                if k > 0 and k < theta_flat.numel():
                    threshold = torch.topk(theta_flat, k, largest=False)[0][-1]
                    self.theta_mask.data = (self.theta_importance > threshold).float()

            # Prune phi - ensure at least one dimension is retained
            if current_pruning_rate > 0:
                phi_flat = self.phi_importance.view(-1)
                k = max(1, int(current_pruning_rate * phi_flat.numel()))
                if k > 0 and k < phi_flat.numel():
                    threshold = torch.topk(phi_flat, k, largest=False)[0][-1]
                    self.phi_mask.data = (self.phi_importance > threshold).float()
...
class PrunedEnsemble_FC_GAMP(EnsembleModule):
    ...
    def update_importance(self):
        """Update weight importance scores"""
        with torch.no_grad():
            self.weight_importance += torch.abs(self.fc.weight)
    ...
    def apply_pruning_mask(self, current_epoch):
        ...
        with torch.no_grad():
            if current_pruning_rate > 0:
                weight_flat = self.weight_importance.view(-1)
                k = max(1, int(current_pruning_rate * weight_flat.numel()))
                if k > 0 and k < weight_flat.numel():
                    threshold = torch.topk(weight_flat, k, largest=False)[0][-1]
                    self.weight_mask.data = (self.weight_importance > threshold).float()

        # Update GAMP module pruning simultaneously
        if self.use_gamp:
            self.gamp_module.apply_pruning_mask(current_epoch)
    ...
    def vae_forward(self, alpha_input):
        """VAE forward propagation"""
        embedded = F.relu(self.encoder_fc1(alpha_input))
        embedded_mean = self.encoder_fcmean(embedded)
        embedded_logvar = self.encoder_fcvar(embedded)
        z_embedded = self.reparameterize(embedded_mean, embedded_logvar)
        alpha_decoded = self.decoder_fc1(z_embedded)

        MSE = F.mse_loss(alpha_decoded, alpha_input)
        KLD = -0.5 * torch.mean(1 + embedded_logvar - embedded_mean.pow(2) - embedded_logvar.exp())

        return alpha_decoded, MSE + 0.1 * KLD
...
class PrunedEnsemble_Conv2d_GAMP(EnsembleModule):
    ...
    def update_importance(self):
        """Update weight importance scores"""
        with torch.no_grad():
            self.weight_importance += torch.abs(self.conv.weight)
